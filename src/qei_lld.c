...
/**
 * @brief   QEID9 driver identifier.
 * @note    The driver QEID9 allocates the timer TIM9 when enabled.
 */
...
void qei_lld_start(QEIDriver *qeip) {
  ...
  switch (qeip->config->mode) {
  case  QEI_MODE_QUADRATURE:
    if (qeip->config->resolution == QEI_BOTH_EDGES)
      qeip->tim->SMCR  = TIM_SMCR_SMS_1 | TIM_SMCR_SMS_0;
    else
      qeip->tim->SMCR  = TIM_SMCR_SMS_0;
    break;
  case QEI_MODE_DIRCLOCK1:
    /* Direction/Clock mode 1.
     * Direction input on TI1, Clock input on TI2. */
    qeip->tim->SMCR  = TIM_SMCR_SMS_0;
    break;
  case QEI_MODE_DIRCLOCK2:
    /* Direction/Clock mode 2.
     * Clock input on TI1, Direction input on TI2. */
